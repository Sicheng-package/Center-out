...
startTask();
    ...
    function startTask()
        ...
        centerActivated   = false;   % 由 applyTrialMeta 决定 active 侧是否需要 hold
        centerEnterTime   = NaN;
        centerexittime    = NaN;
        hoverEnterTime    = NaN;
        firstMove         = true;
        ...
        while true
            ...
            switch op
                ...
                case 6
                    rTrial = double(typecast(uint8(payload(1:2)),'uint16'));
                    trialIndex = rTrial;
                    m  = double(payload(3));  % trialMode
                    d  = double(payload(5));  % direction
                    applyTrialMeta(m,d);
    ...
    function resetTarget()
        ...
        centerexittime   = NaN;
        centerExitPos    = [0 0];
        centerActivated  = false;    % 等待新 trialMeta 决定
        targetActivated  = false;
        firstMove        = true;
        localCenterDone  = false;
        remoteCenterDone = false;
    ...
    function applyTrialMeta(m,dir)
        ...
        % L 端：THIS_SIDE = 1
        % uniL：主动；uniR：被动；bi：主动
        if trialMode == 2      % uniL
            setPassiveOverlay(false);   % active
            showTargetCircles(0);       % 只显示 center
            targetActivated = false;
        elseif trialMode == 3  % uniR
            setPassiveOverlay(true);    % passive，全黑
            showTargetCircles(0);
            targetActivated = false;
        else                   % bi
