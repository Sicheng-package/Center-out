...
startTask();
    ...
    function planNextTrial()
        if trialIndex > 0
            ...
            if ~l3Ready
                disp('Warning: L5 handshake timeout.');
            end
        end
        ...
        switch lower(tag)
            case 'bi'
                trialMode     = 1;
                uniActiveSide = 0;
                if isempty(remainingBI), remainingBI = targetSequence(:)'; end
                dir = remainingBI(1);
    ...
    function endTrialFail(sendRemoteFail)
        if failInProgress
            return;
        end
        failInProgress = true;
        ...
        if sendRemoteFail && ~isempty(tcpObj) && isvalid(tcpObj)
            write(tcpObj, uint8(3), 'uint8');
        ...
        if ~centerActivated && isActiveSide()
            if targetActivated && inTarget(x,y,currentTarget)
                ...
                elseif ~targetReachedOnce && tNow >= hoverEnterTime + targetHoldDuration
                    ...
                    localSuccess = true;
                    if trialMode == 1
                        if remoteSuccess, compareTtlAndProceed(); end
                    else
                        doBothSuccess();
                    end
                end
    ...
    function resetForNextTrial()
        ...
        touchStable = 0; touchHiCnt = 0; touchLoCnt = 0;
        if tcpObj.NumBytesAvailable > 0
            bytes = read(tcpObj, tcpObj.NumBytesAvailable, 'uint8');
            keep = bytes(bytes==99 | bytes==10 | bytes==11);
            if ~isempty(keep)
                rxbuf = [rxbuf; keep(:)];
            end
        end
