...
TOUCH_FRAMES       = 10;
COMMIT_RETRY_INTERVAL   = 0.3;
COMMIT_HARD_TIMEOUT     = 8.0;
...
startTask();
        ...
        while true
            ...
            switch op
                ...
                case OP_TARGET_ON
                    ...
                    goMs   = double(typecast(uint8(payload(4:5)), 'uint16'));
                    goTime = goMs / 1000;
                    safeWrite(tcpObj, [uint8(OP_TARGET_ACK), typecast(uint16(rTrial),'uint8')], 'uint8', 'TARGET_ACK');
                    if rTrial == trialIndex && ~trialFinalized
                        if targetSyncRecvForTrial ~= rTrial
                            targetSyncRecvForTrial = rTrial;
                            scheduleTargetActivation(tgt, goTime, rTrial);
                        end
                    end
    ...
    function sendFail(statusCode)
        ...
        if ~awaitingCommit || isnan(awaitingCommitStartTime)
            awaitingCommitStartTime = toc(cycleStartTime);
        end
        awaitingCommit = true;
        awaitingCommitSince = toc(cycleStartTime);
