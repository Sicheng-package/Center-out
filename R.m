...
startTask();
    ...
    function serverPlanAndBroadcastTrial()
        ...
        switch lower(tag)
            ...
            case 'unil'
                ...
                if isempty(remainingUniL)
                    remainingUniL = targetSequence(randperm(4));
                end
                dir = remainingUniL(1);
            ...
            case 'unir'
                ...
                if isempty(remainingUniR)
                    remainingUniR = targetSequence(randperm(4));
                end
                dir = remainingUniR(1);
    ...
    function updateQueuesOnOutcome(successFlag)
        if ~successFlag
            return;
        end
    ...
    function advanceScheduleOnSuccess()
    ...
    function doBothSuccess()
        ...
        updateQueuesOnOutcome(true);
        advanceScheduleOnSuccess();
        ...
        if centerActivated
            if inCenter(x,y)
                ...
                elseif tNow >= centerEnterTime + centerHoldDuration
                    ...
                    safeWrite(tcpObj,[uint8(4), typecast(uint16(trialIndex),'uint8')],'uint8');
            ...
            if trialMode == 1 || (trialMode == 2 && THIS_SIDE == 1) || (trialMode == 3 && THIS_SIDE == 2)
    ...
    function checkFirstMove()
        ...
        if isempty(aboveThresholdCount)
        ...
        if firstMove && ~centerActivated && targetActivated && ...
           (trialMode == 1 || (trialMode == 2 && THIS_SIDE == 1) || (trialMode == 3 && THIS_SIDE == 2))
            ...
            if norm(pos - centerExitPos) > FIRSTMOVE_DELTA
                ...
                if aboveThresholdCount >= FIRSTMOVE_FRAMES
    ...
    function tf = inTarget(x,y,targetNum)
        ...
        cx = targetCenters(targetNum,1);
        cy = targetCenters(targetNum,2);
