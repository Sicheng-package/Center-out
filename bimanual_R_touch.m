...
TOUCH_FRAMES       = 10;
TARGET_SYNC_LEAD   = 0.01;
TARGET_SYNC_RETRY_INTERVAL = 0.02;
TARGET_SYNC_MAX_TRIES      = 300;
CENTER_ACK_RETRY_INTERVAL  = 0.02;
...
targetSyncAckForTrial = 0;
targetSyncLastSendTime = NaN;
targetSyncSendTries = 0;
targetSyncGoTime = NaN;
targetSyncTarget = 0;
...
startTask();
    ...
    function L = payloadLen(op_)
        switch op_
            ...
            case OP_TARGET_ON,   L = 5;
            case OP_COMMIT,      L = 3;
            case OP_TARGET_ACK,  L = 2;
            case OP_READY,       L = 0;
            case OP_DONE,        L = 0;
            case OP_STOP,        L = 0;
            otherwise,           L = 0;
            ...
            switch op
                ...
                case OP_TARGET_ACK
                    ...
                    if ackTrial == trialIndex
                        targetSyncAckForTrial = ackTrial;
                    end
    ...
    function recordMousePos(~, ~)
        ...
        maybeSendTargetSync();
        maybeActivateScheduledTarget(t);
        if trialFinalized || sessionEnding, return; end
    ...
    function scheduleTargetActivation(tgt, goTime, trialNum)
        if trialFinalized || sessionEnding, return; end
        if trialNum ~= trialIndex, return; end
        pendingTargetTrial  = trialNum;
        pendingTarget       = double(tgt);
        pendingTargetGoTime = goTime;
    end

    function maybeActivateScheduledTarget(tNow)
        if trialFinalized || sessionEnding, return; end
        if isnan(pendingTargetGoTime), return; end
    ...
    function sendTargetSyncPacket()
        if targetSyncSentForTrial ~= trialIndex, return; end
        if targetSyncTarget <= 0 || isnan(targetSyncGoTime), return; end
        targetGoMs = uint16(min(65535, max(0, round(targetSyncGoTime * 1000))));
        payload = [uint8(OP_TARGET_ON), typecast(uint16(trialIndex),'uint8'), uint8(targetSyncTarget), typecast(targetGoMs,'uint8')];
        safeWrite(tcpObj, payload, 'uint8', 'TARGET_ON');
        targetSyncLastSendTime = toc(cycleStartTime);
        targetSyncSendTries = targetSyncSendTries + 1;
    end
    ...
    function maybeSendTargetSync()
        ...
        if targetSyncAckForTrial == trialIndex, return; end

        if targetSyncSentForTrial ~= trialIndex
            if isempty(remainingTargets)
                remainingTargets = targetSequence(randperm(length(targetSequence)));
            end
            targetSyncTarget = remainingTargets(1);
            targetSyncGoTime = toc(cycleStartTime) + TARGET_SYNC_LEAD;
            targetSyncSentForTrial = trialIndex;
            targetSyncAckForTrial = 0;
            targetSyncLastSendTime = NaN;
            targetSyncSendTries = 0;
            scheduleTargetActivation(targetSyncTarget, targetSyncGoTime, trialIndex);
        end

        if targetSyncSendTries >= TARGET_SYNC_MAX_TRIES, return; end
        if ~isnan(targetSyncLastSendTime) && ...
                (toc(cycleStartTime) - targetSyncLastSendTime) < TARGET_SYNC_RETRY_INTERVAL
            return;
        end
        sendTargetSyncPacket();
    end
    ...
    function activateTargetSynced(tgt)
        ...
        targetOnTime = toc(cycleStartTime);
        firstMove = true;
        if isempty(mouseDot) || ~isvalid(mouseDot)
    ...
    function recordData = saveMousePathAndTrueCoordinates(pathData, cyc, totalTime, status)
        ...
        folderName = fullfile(pwd,'S2_R');
        if ~exist(folderName,'dir'), mkdir(folderName); end
        traceFile = fullfile(folderName, ['right' num2str(cyc) '.xlsx']);
        writematrix(pathData, traceFile);
        trialData = [cyc, targetDir, totalTime, lastMoveTime, targetOnTime, status, ...
