...
global cyclesCompleted maxCycles mousePath timerObj rxbuf;
...
startTask();
    ...
    function cleanup()
        ...
        try write(daqSession,[1 1 0 0 0]); catch, end
        try delete(daqSession); catch, end
        try delete(tcpObj); catch, end
        delete(fig);
    end
    ...
    function startTask()
        ...
        modes = {'Mix (Bi/L/R)', 'Bi-Manual Only', 'Uni-Left Only', 'Uni-Right Only'};
        [s,v] = listdlg('PromptString','Select Task Mode:',...
                        'SelectionMode','single','ListString',modes);
        if v
            switch s
                case 1, BASE_PATTERN = {'bi','uniL','uniR'};
                case 2, BASE_PATTERN = {'bi'};
                case 3, BASE_PATTERN = {'uniL'};
                case 4, BASE_PATTERN = {'uniR'};
            end
        else
            BASE_PATTERN = {'bi'}; 
        end
    ...
    function planNextTrial()
        if trialIndex > 0
            ...
            while ~l3Ready && toc(waitStart) < 1.5
                if tcpObj.NumBytesAvailable > 0
                    val = read(tcpObj, 1, 'uint8');
                    if val == 99
                        l3Ready = true;
                        break; 
                    end
            ...
            if ~l3Ready
                disp('Warning: L3 handshake timeout.');
            end
        end
    ...
    function recordMousePos(~,~)
        ...
        persistent loopCount lastTouch
        if isempty(loopCount), loopCount = 0; lastTouch = 0; end
        loopCount = loopCount + 1;
        logicTouch = lastTouch;
        logTouch   = NaN; 
        ...
        mousePath = [mousePath; x, y, t, logTouch];
        checkFirstMove();
        logic(x, y, logicTouch);
        checkContraMove(x,y);
        checkTimeout();
        try
            checkRemoteSignal();
            ...
            if inCenter(x,y) && (touchVal == 1)
                ...
                elseif tNow >= centerEnterTime + centerHoldDuration
                    ...
                    if isActiveSide()
                        ...
                        safeWrite(tcpObj,[uint8(7)],'uint8');
                    end
                end
            else
                set(mouseDot, 'Color', 'r');
                if ~isnan(centerEnterTime)
                    sendSignal(tcpObj,3);
                    if ~inCenter(x,y)
                        failStatus = 5; % SlipOut
                    elseif touchVal == 0
                        failStatus = 4; % LiftHand
                    else
                        failStatus = 5;
                    end
                    doBothFail();
        ...
        while true
            ...
            switch op
                ...
                case 4
                    ...
                    if trialMode == 1 && localCenterDone && isActiveSide()
                        showTargetCircles(currentTarget);
                        targetActivated = true;
                        safeWrite(tcpObj,[uint8(7)],'uint8');
                    end
