...
startTask();
    ...
    function showBlackPanelScreenSync(duration)
        ...
        blk = uipanel('Parent',fig,'Units','normalized','Position',[0 0 1 1],'BackgroundColor','black','BorderType','none');
        drawnow;
    ...
    function startTask()
      % targetSequence = [7 3 1 5 2 4 6 8];
        ...
        trialMode = 1;
        localSuccess=false;
        fig = figure('Color','black','Pointer','custom','Units','normalized','Position',[0 0 1 1],'MenuBar','none','ToolBar','none','WindowState','fullscreen');
        ax = axes('Parent',fig,'Color','black','Units','normalized','Position',[0 0 1 1],'DataAspectRatio',[1 1 1]);
        daqSession = daq("ni");
        ...
        axis off; hold on;
        set(fig,'WindowButtonMotionFcn',@mouseMoved);
        passiveOverlay = uipanel('Parent',fig,'Units','normalized','Position',[0 0 1 1],'BackgroundColor','black','BorderType','none','Visible','off');
    ...
    function drawCircles()
        ...
        for i=1:8
            ...
            rectangle('Position',[x-circleDiameter/2,y-circleDiameter/2,circleDiameter,circleDiameter],'Curvature',[1,1],'EdgeColor','w','FaceColor','k','LineWidth',3,'UserData',i,'Visible','off');
        end
        rectangle('Position',[-circleDiameter2/2, -circleDiameter2/2, circleDiameter2, circleDiameter2],'Curvature',[1,1],'EdgeColor','y','FaceColor','y','LineWidth',3,'Visible','on');
        xlim([-0.5,0.5]);
        ylim([-0.5,0.5]);
    end
    ...
    function applyTrialMeta(mode_, dir)
        trialMode = mode_;
        currentTarget = dir;
            ...
            if targetActivated
                if inTarget(x,y,currentTarget)
                    ...
                    elseif ~targetReachedOnce && tNow >= hoverEnterTime + targetHoldDuration
                        ...
                        sendTTL(2);
                        localSuccess = true;
                        payload = uint8([2; typecast(uint16(trialIndex),'uint8')'; typecast([ttlFirstMove, ttlEnd],'uint8')']);
                        safeWrite(tcpObj,payload,'uint8');
                        if trialMode == 1
                            if remoteSuccess
                                compareTtlAndProceed();
    ...
    function showTargetCircles(targetNum)
        ...
        objs = objs(:);
        fc = get(objs,'FaceColor'); if ~iscell(fc), fc={fc}; end
        ...
        for i=1:numel(circleObjs)
            ...
            if idx == targetNum
                set(circleObjs(i),'Visible','on','FaceColor','w','EdgeColor','w','LineWidth',3);
            else
    ...
    function recordData = saveMousePathAndTrueCoordinates(pathData, trialID, totalTime, status)
        ...
        trialData = [trialID, targetDir, totalTime, lastMoveTime, status, ttlFirstMove, ttlEnd, ttlStartDiff, ttlEndDiff, centerexittime, trialMode];
        summaryFile = fullfile(folderName, 'Summary_L.xlsx');
        if ~isfile(summaryFile)
            header = {'Trial','Target','Dur','FirstMoveTime','Status','TTL_FirstMove','TTL_End','TTL_StartDiff','TTL_EndDiff','CenterExitTime','Mode(1=BI,2=L,3=R)'};
